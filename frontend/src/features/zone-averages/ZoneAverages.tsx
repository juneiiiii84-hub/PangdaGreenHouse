...
const getGreenhouseSummary = (avg: any) => {
  if (!avg) return '📴 ยังไม่มีข้อมูลจากเซ็นเซอร์ในโรงเรือน';
  ...
  let tempText = '';
  if (temp > 35) tempText = '🌡️ อุณหภูมิ: ร้อนจัดมาก (ระวังใบไม้ไหม้เกรียม)';
  else if (temp >= 31) tempText = '🌡️ อุณหภูมิ: ค่อนข้างร้อน (แนะนำเปิดพัดลมระบายอากาศ)';
  else if (temp >= 25) tempText = '🌡️ อุณหภูมิ: กำลังอุ่นสบายพอเหมาะ ต้นไม้เติบโตได้ดีเยี่ยม';
  else if (temp >= 20) tempText = '🌡️ อุณหภูมิ: ค่อนข้างเย็นสบาย (พืชอาจโตช้าลงเล็กน้อย)';
  else tempText = '🌡️ อุณหภูมิ: อากาศหนาวเกินไป (พืชอาจหยุดชะงักการเติบโต)';

  let humText = '';
  if (hum > 90) humText = '💧 ความชื้น: ชื้นแฉะเกินเกณฑ์ (ระวังโรคราใบไม้ระบาด)';
  else if (hum >= 81) humText = '💧 ความชื้น: ค่อนข้างสูง (อากาศเริ่มหนาแน่นอบอ้าว)';
  else if (hum >= 60) humText = '💧 ความชื้น: กำลังดีพอดีมาก ต้นไม้ชอบดูดปุ๋ยได้ดีที่สุด';
  else if (hum >= 40) humText = '💧 ความชื้น: อากาศค่อนข้างแห้ง (พืชจะคายน้ำเร็วขึ้น)';
  else humText = '💧 ความชื้น: อากาศแห้งแล้งมาก (ควรเปิดพ่นหมอกหรือรดน้ำเพิ่มความชื้น)';

  let vpdText = '';
  if (vpd > 1.2) vpdText = '💨 การคายน้ำ: น้ำระเหยเร็วเกินไป ต้นไม้อาจปิดปากใบทำให้ขาดสารอาหาร';
  else if (vpd >= 0.4 && vpd <= 0.8) vpdText = '💨 การคายน้ำ: คายน้ำสมบูรณ์แบบ รากดูดซึมปุ๋ยขึ้นยอดได้เต็มที่ร้อยเปอร์เซ็นต์';
  else if (vpd < 0.4) vpdText = '💨 การคายน้ำ: อากาศอ้าวเกินจนคายน้ำไม่ได้ พืชไม่ดูดกินปุ๋ย';
  else vpdText = '💨 การคายน้ำ: ระเหยน้ำในระดับปกติ ต้นไม้เติบโตเรื่อยๆ';

  let lightText = '';
  if (lux > 43200) {
    lightText = '☀️ แสงสว่าง: ความเข้มแสงแดดจัดจ้าเกินเกณฑ์มาตรฐาน ควรดึงสแลนกรองแสงลงเพื่อป้องกันผิวใบพืชไหม้เสียหายจากความร้อนสะสม';
  } else if (lux >= 21600) {
    lightText = '☀️ แสงสว่าง: ความเข้มแสงอยู่ในเกณฑ์ปกติเหมาะสม พืชสังเคราะห์แสงได้ดีตามธรรมชาติ';
  } else {
    lightText = '☀️ แสงสว่าง: ความเข้มแสงต่ำกว่าเกณฑ์มาตรฐาน ควรดำเนินการม้วนสแลนพรางแสงออกเพื่อรับแสงแดดธรรมชาติเพิ่มขึ้น หรือหากเป็นช่วงสภาพอากาศปิดฝนตก ควรเปิดระบบไฟช่วยปลูก (Grow Light) เพื่อเพิ่มความเข้มแสงให้แก่พืชโดยตรง';
  }
